...
# def transform_dct(img, quality=100):
#     # print(f'Initial image size: {len(img)}')
#     # y, cbcr = img  
#     #(224,224,3)
#     if img.dtype != 'uint8':
#         img = np.ascontiguousarray(img, dtype='uint8')
...
#     img_ycrcb = cv2.cvtColor(decoded_img, cv2.COLOR_BGR2YCrCb)
#     y, cr, cb = cv2.split(img_ycrcb)
#     print(f'after convert to ycrcb by cv2,y shape{y.shape}, cr shape{cr.shape}')

#     # Define a function to perform DCT
#     def perform_dct(channel):
#         return dct(dct(channel.T, norm='ortho').T, norm='ortho')

#     # Perform DCT on each channel
#     dct_y = perform_dct(y)
#     dct_cb = perform_dct(cb)
#     dct_cr = perform_dct(cr)
#     print(f'transform_dct size dct_y{dct_y.shape}, dct_cb{dct_cb.shape}')

#     return dct_y, dct_cb, dct_cr

# def transform_dct(img, quality=100):
#     # 确保输入图像是 uint8 类型
#     assert img.dtype == np.uint8, "输入图像的类型必须是 uint8"
    
#     # 确保输入图像的尺寸是 (448, 448, 3)
#     assert img.shape == (448, 448, 3), "输入图像的尺寸必须是 (448, 448, 3)"
...
#     # # 对每个通道进行 8x8 块的 DCT 变换
#     # dct_y = np.zeros_like(y, dtype=np.float32)
#     # dct_cr = np.zeros_like(cr, dtype=np.float32)
#     # dct_cb = np.zeros_like(cb, dtype=np.float32)
    
#     # # 遍历 8x8 块并进行 DCT 变换
#     # for i in range(0, y.shape[0], 8):
#     #     for j in range(0, y.shape[1], 8):
#     #         dct_y[i:i+8, j:j+8] = block_dct(y[i:i+8, j:j+8])
#     #         dct_cr[i:i+8, j:j+8] = block_dct(cr[i:i+8, j:j+8])
#     #         dct_cb[i:i+8, j:j+8] = block_dct(cb[i:i+8, j:j+8])
#     num_blocks_y = y.shape[0] // 8
#     num_blocks_x = y.shape[1] // 8
    
#     # 初始化 (64, num_blocks_y, num_blocks_x) 的输出矩阵
#     dct_y_blocks = np.zeros((64, num_blocks_y, num_blocks_x), dtype=np.float32)
...
#             for k in range(64):
#                 row, col = divmod(k, 8)
#                 dct_y_blocks[k, i//8, j//8] = dct_block[row, col]
#     import logging
#     from pprint import pformat

# # 配置日志记录
#     # logging.basicConfig(filename='dct_y_blocks_log.txt', level=logging.DEBUG, format='%(message)s')

# # 将 dct_y_blocks 数据格式化为字符串
#     # dct_y_blocks_str = pformat(dct_y_blocks)

# # 记录格式化后的数据
#     # logging.debug(f'after dct_y{dct_y_blocks_str}')
#     # print(f'after dct dct_y{dct_blocks.shape}')
#     # print(f'after dct_y{dct_y_blocks}')

#     return dct_y_blocks#[64,56,56]

#     print(f'after dct dct_y{dct_y.shape},cr{dct_cr.shape}, cb{dct_cb.shape}')
    
#     return dct_y, dct_cr, dct_cb


# # Example usage
# img = cv2.imread('path_to_image.jpg')  # Load an image
# dct_y, dct_cb, dct_cr = transform_dct(img)
...
def transform_dct(img, quality=100):
    ...
    # 初始化 (64, num_blocks_y, num_blocks_x) 的输出矩阵
    dct_y_blocks = np.zeros((64, num_blocks_y, num_blocks_x), dtype=np.float32)
    dct_cr_blocks = np.zeros((64, num_blocks_y, num_blocks_x), dtype=np.float32)
    dct_cb_blocks = np.zeros((64, num_blocks_y, num_blocks_x), dtype=np.float32)
    ...
    for i in range(0, y.shape[0], 8):
        for j in range(0, y.shape[1], 8):
            ...
            dct_y_block = block_dct(y_block)
            dct_cr_block = block_dct(cr_block)
            dct_cb_block = block_dct(cb_block)
            
            for k in range(64):
                row, col = divmod(k, 8)
                dct_y_blocks[k, i//8, j//8] = dct_y_block[row, col]
                dct_cr_blocks[k, i//8, j//8] = dct_cr_block[row, col]
                dct_cb_blocks[k, i//8, j//8] = dct_cb_block[row, col]

    dct_y_blocks = torch.from_numpy(dct_y_blocks)
    dct_cr_blocks = torch.from_numpy(dct_cr_blocks)
    dct_cb_blocks = torch.from_numpy(dct_cb_blocks)

    
    return dct_y_blocks, dct_cr_blocks, dct_cb_blocks
...
def transform_dct1(img):
    ...
    # print(f'ycbcr size{ycbcr_image.shape}')(448,448,3)
    # num_batchsize = ycbcr_image.shape[0]
    size = ycbcr_image.shape[1]
    ycbcr_image = torch.from_numpy(ycbcr_image)
    # ycbcr_image = ycbcr_image.reshape( 3, size // 8, 8, size // 8, 8).permute(0, 2, 4, 1, 3, 5)
    ycbcr_image = ycbcr_image.reshape( 3, size // 8, 8, size // 8, 8).permute(1, 3, 0, 2, 4)
    ycbcr_image = DCT.dct_2d(ycbcr_image, norm='ortho')
    # ycbcr_image = ycbcr_image.reshape(size // 8, size // 8, -1).permute(0, 3, 1, 2)
    ycbcr_image = ycbcr_image.reshape(size // 8, size // 8, -1).permute(2, 0, 1)
    return ycbcr_image
...
def to_tensor_dct(img):
    ...
    """
    # print("Original image shape:", img.shape)#(224,3)
...
def upscale(img, upscale_factor=None, desize_size=None, interpolation='BILINEAR'):
    ...
    if upscale_factor is not None:
        ...
        raise ValueError
    a=cv2.resize(img, dsize=(dw, dh), interpolation=INTER_MODE[interpolation])
    # print(f'item dtype of upscale{a.shape}')
...
def normalize(tensor, mean, std):
    ...
    # print(f'before normalize data {tensor}')
    # print(f'before normalize data type {type(tensor)}')
    # print(f'before normalize data shape {tensor.shape}')
