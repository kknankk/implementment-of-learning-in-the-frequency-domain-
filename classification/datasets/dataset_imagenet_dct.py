...
def opencv_loader(path, colorSpace='YCrCb'):
    ...
    # print(f'opencv_loader  size {image.shape}')
...
class DatasetFolderDCT(VisionDataset):
    ...
    def __getitem__(self, index):
        ...
        # if sample is None :
        #     raise ValueError(f'Index {index} returned None for sample')
        # if  target is None:
        #     raise ValueError(f'Index {index} returned None for  target')

        # path = '/storage-t1/user/kaixu/datasets/ILSVRC2012/test/train/n02447366/n02447366_23489.JPEG'
        # print('{}, {}'.format(index, path))
        if self.backend == 'opencv':
            # print(f'loader {self.loader}')
            # print(f'loader {self.loader.__name__}')  # 获取loader的名称
            sample = self.loader(path, backend='opencv', colorSpace='BGR')
            # print(f'getitem sample {sample.shape}')
        ...
        # else:
        #     if dct_cb is not None:
        #         # print(f'dct_cb is not none')
        #         # print
        #         image = torch.cat((dct_y, dct_cb, dct_cr), dim=0)

        #         return image, target
        #     else:
        #         print(f'dct_cb is none')
...
#         train_dataset = ImageFolderDCT('/data/ke/tiny-imagenet-200/train', transforms.Compose([
...
#     #     print(f'dataloaer image size{image.shape}')#[1,196,56,56]
#     #     print(f' dataloader target size{targets.shape}')#[1]
#     # #     coef = dct_y.numpy()
#     #     print(f'after train_loader dct_y shape {dct_y.shape}') #[1,64,56,56]
