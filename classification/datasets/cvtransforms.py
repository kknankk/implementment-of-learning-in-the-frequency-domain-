...
class Upscale(object):
    ...
    def __call__(self, img):
        # print(f' after upscale{img.shape}, F.upscale {F.upscale(img, self.upscale_factor, self.interpolation)}')
        upscale=F.upscale(img, self.upscale_factor, self.interpolation)
        # return img, upscale
        return upscale
...
class TransformUpscaledDCT(object):
    ...
    def __call__(self, img):
        ...
        dct_y, dct_cb,dct_cr = F.transform_dct(img)
        # _, dct_cb, dct_cr = F.transform_dct(cbcr)
        # print(f'Output size dct_y{dct_y.shape}, dct_cb{dct_cb.shape}, dct_cr{dct_cr.shape}')
        return dct_y, dct_cb, dct_cr

class Transform_torchdct(object):
    def __init__(self):
        pass

    def __call__(self, img):
        # y, cb,cr = img[0], img[1],img[2]
        img = F.transform_dct1(img)
        # _, dct_cb, dct_cr = F.transform_dct(cbcr)
        # print(f'Output size dct_y{dct_y.shape}, dct_cb{dct_cb.shape}, dct_cr{dct_cr.shape}')
        return img
...
# 设置基本配置，日志级别为 DEBUG，日志格式包含时间、日志级别和消息
# logging.basicConfig(level=logging.DEBUG, format='%(asctime)s - %(levelname)s - %(message)s')
logging.basicConfig(level=logging.DEBUG, 
                    format='%(asctime)s - %(levelname)s - %(message)s',
                    filename='dct_transform.log',  # 日志文件路径
                    filemode='w')
...
class ToTensorDCT(object):
    ...
    def __call__(self, img):
        ...
        # print(f'before to_tensor img{img.shape}')#[224,224,3]
        # y, cb, cr = img[0], img[1], img[2]
        y, cb, cr = img[0:1], img[1:2], img[2:3]
        # print(f'before to_tensor y{y.shape}')#[224,3]
        y, cb, cr = F.to_tensor_dct(y), F.to_tensor_dct(cb), F.to_tensor_dct(cr)
        y = torch.tensor(y)
        cb = torch.tensor(cb)
        cr = torch.tensor(cr)
        # print(f'Type of y before normalization: {y.shape}')
        # print(f'Type of cb before normalization: {cb.shape}')
        # print(f'Type of cr before normalization: {cr.shape}')
        torch.set_printoptions(threshold=np.inf)
        # logging.debug(f'Type of y before normalization: {y}')
        # logging.debug(f'Type of cb before normalization: {cb}')
        # logging.debug(f'Type of cr before normalization: {cr}')
...
class SubsetDCT(object):
    def __init__(self, channels=64):
        ...
        # print("Keys in subset_channel_index:", self.subset_channel_index.keys()) #dict_keys(['6', '24', '32', '48', '64'])
        # print(f"Type of channels: {type(self.subset_channel_index.keys())}")
    ...
    def __call__(self, tensor):
        ...
        # print(f'after subset dct_y{dct_y.shape}, dct_cb{dct_cb.shape}, dct_cr{dct_cr.shape}')
        # print(f'after subset dtype dct_y{dct_y.dtype}, dct_cb{dct_cb.dtype}, dct_cr{dct_cr.dtype}')
        return dct_y, dct_cb, dct_cr

class Aggregate(object):
    def __call__(self, img):
        dct_y, dct_cb, dct_cr = img[0], img[1], img[2]
        # print(f'before Aggregate size dct_y{dct_y.shape},dct_cb{dct_cb.shape},dct_cr{dct_cr.shape}')
        dct_y = torch.cat((dct_y, dct_cb, dct_cr), dim=0)
        # print(f'after Aggregate img size{img.shape}')
        return img
...
class NormalizeDCT(object):
    ...
    def __call__(self, tensor):
        ...
        if isinstance(tensor, list):
            # print(f'normalize is tensor list')
            y, cb, cr = tensor[0], tensor[1], tensor[2]
            # print(f'before normalize y size{y.shape}')
            ...
            # print(f'final size y{y.shape},cb{cb.shape},cr{cr.shape}')#[64,56,56]
            merged_tensor = torch.cat((y, cb, cr), dim=0)
    
            # print(f'merged tensor size {merged_tensor.shape}')  # [192,56,56]
            return y, cb, cr
            # return y, cb, cr
        else:
...
class RandomResizedCrop(object):
    ...
    def __call__(self, img):
        ...
        """
        # print(f'before randomresizecrop image{img.shape}')
